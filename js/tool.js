...
var rgb2hex = function(B, A, C) {
    return ((B << 16) | (A << 8) | C).toString(16)
};
var Limit = function(){
    this.prestr="";
    this.nowstr="";
...
Limit.prototype={
    init:function(a,b){
        var dom = a ;//获取dom
        var maxlen = b;//获取最大字符数
        this.nowstr = dom.value;
        var str = dom.value;
        var _count = str.length;//字符串长度
        var count = str.replace(/[^\x00-\xff]/g,"yy").length;//字符数量
        this.prestr = dom.value = count>maxlen?this.prestr:this.nowstr;
    }
}
...
// 11111111|0000000000
//B<<16 == (B).toString(2)|0000000000000000,B向左移动16次，每次后面加1个0，总共加了16个0
//B|A表示将B和A都转为2进制数，再相加